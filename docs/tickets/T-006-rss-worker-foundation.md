...
### PR #23 — HTML 본문 추출 + 임베딩 + 클러스터링

- `apps/worker/extract.py` — readability-lxml 또는 trafilatura로 본문 추출 (ADR-013 헤드라인 본론 추출 정합)
  - 본문은 RAM에서만 처리, DB 저장 X (CLAUDE.md rule 2). LLM 입력으로만 전달
- `apps/worker/embed.py` — sentence-transformers 또는 OpenAI ada-002 (ADR-003)
  - embedding은 worker RAM에서 클러스터링 즉시 사용 후 폐기 (CLAUDE.md rule 4)
- `apps/worker/cluster.py` — HDBSCAN (sklearn) 또는 agglomerative
...
- `apps/worker/summarize.py` — Gemini 2.0 Flash (ADR-014 Phase 1) — why_trending + coverage_summary
...
- `apps/worker/trust_signal.py` — Trust Signal LLM (prompt_version `trust_signal_v1`)
  - hoax_likelihood / clickbait_score / is_investment 평가
  - human_review_required 자동 큐 (ADR-015 A2.7 — 정치/선거/범죄/사망/재난/의료/금융 카테고리)
- `apps/worker/cost_monitor.py` — 일일 비용 누적 + $50/월 cap 도달 시 Slack alert (ADR-014)
