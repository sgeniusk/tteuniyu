...
## Amendment 1 (2026-05-09, P0w D7+1) — superseded by ADR-015

> **Status**: Superseded
> **Superseder**: ADR-015 §2.3 "이념 색상 완전 중립화"
> **Decider**: 태욱 (Founder)

### 변경 요약

ADR-009는 이념 라벨을 "보조 mini panel"로 강등했다. v1.7.1 patch에서 한 단계 더 진행해 **모든 색·기호 라벨을 surface에서 완전 제거**한다. 자세한 내용은 ADR-015 §2.3을 참고한다.

### 폐기 항목

- `<AuxiliaryStancePanel>` 컴포넌트 삭제 (apps/web/components/AuxiliaryStancePanel.tsx)
- 매체 dot 4색 → 단일 회색 (slate-400) 통일
- 4분류 라벨 (보수/진보/중도/공영) 텍스트도 제거 — 매체명만 노출
- "이념" / "stance" / "ideology" 단어 코드/UI/문서에서 추방 (CLAUDE.md Naming Ban List 추가)

### 대체 정보

이념 색상 자리에 ADR-015 Trust Tag 3종 (`hoax` / `clickbait` / `low_confidence`) + Investment Risk Tag가 들어간다. 정치적 분류가 아닌 **활용 가능한 위험 신호**로 surface 가치를 대체한다.

### 근거

- 사용자 지시 (2026-05-09) — "좌 우 매체에 대한거(색 차이라든지 그런거)는 그냥 없애도 될 것 같고"
- ADR-010 정신 일관성 — Pro 분석 영역에 한정도 정치적 비난 위험 동일
- 한국 법무 — 이념 라벨 자체가 명예훼손·허위사실 유포 리스크

**End of Amendment 1 — 2026-05-09**
