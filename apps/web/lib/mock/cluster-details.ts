/**
 * Mock cluster detail content — v1.6.5 patch.
 *
 * For each of the 15 mock clusters in `lib/mock/clusters.ts`, supplies:
 *  - `ai_analysis.why_trending` + `coverage_summary` (always)
 *  - `ai_analysis.entity_card` (9 clusters where the subject is a named
 *    org/event; route handler nests this under `ai_analysis.deep`)
 *  - `outlet_reports` (4~7 fabricated headlines per cluster)
 *  - `youtube_news` (11 clusters; placeholder video_id with fallback UI
 *    for P0w; P0a will swap with YouTube Data API picks via T-006 worker)
 *
 * P0a (T-005 + T-006) replaces:
 *  - `ai_analysis.*` with Claude Haiku output (same JSON shape) +
 *    copy-rate ≤ 15% on `coverage_summary`.
 *  - `outlet_reports` with rows from `articles` table.
 *  - `youtube_news` with real video_id picked from MBC/YTN/연합뉴스 channels.
 *  - trend (4-window) is generated at request time (lib/mock/trend.ts) for
 *    P0w; P0a reads from `keyword_trends` table populated by the worker.
 */

import type { OutletStance } from '@/lib/mock/outlets'
import type { EntityCard, YouTubeNews } from '@/lib/api/cluster-schemas'
...
export interface ClusterDetail {
  ai_analysis: {
    ...
    /** Optional, ~9 of 15 clusters carry entity definition + facts. */
    entity_card?: EntityCard
  }
  outlet_reports: readonly OutletReport[]
  /** Optional, 11 of 15 clusters carry a placeholder MBC/YTN/연합뉴스 video. */
  youtube_news?: YouTubeNews
}
...
export const CLUSTER_DETAILS: Readonly<Record<string, ClusterDetail>> = {
  ...
  '00000000-0000-4000-8000-000000000001': {
    ...
    youtube_news: {
      video_id: 'YTN-news-01',
      title: '[YTN24] 美 연준, 5월 금리 동결 시사…환율 반응은?',
      channel: 'YTN',
    },
    outlet_reports: [
  ...
  '00000000-0000-4000-8000-000000000002': {
    ...
    youtube_news: {
      video_id: 'MBC-news-02',
      title: '[MBC뉴스] 국민연금 개편안 발표…4분의 1 가입자 영향',
      channel: 'MBC뉴스',
    },
    outlet_reports: [
  ...
  '00000000-0000-4000-8000-000000000003': {
    ...
    youtube_news: {
      video_id: 'MBC-news-03',
      title: '[MBC뉴스] 서울시 청년 월세 30만원으로 확대',
      channel: 'MBC뉴스',
    },
    outlet_reports: [
  ...
  // 4. OpenAI 신규 한국어 모델 공개 — tech_science (entity_card)
  '00000000-0000-4000-8000-000000000004': {
    ai_analysis: {
      ...
      entity_card: {
        ...
        domain_facts: [
          { label: '추정 기업가치', value: '약 $157B', source: 'Bloomberg 2024-10' },
          { label: 'CEO', value: 'Sam Altman' },
          { label: '주요 제품', value: 'ChatGPT, GPT-4, Sora' },
          { label: '직원 수', value: '약 2,000명', source: 'TechCrunch 2025-Q1' },
          { label: '최근 모금', value: '$6.6B Series Round', source: '공개 자료 2024-10' },
        ],
      },
    },
    youtube_news: {
      video_id: 'YHA-news-04',
      title: '[연합뉴스] OpenAI, 한국어 전용 모델 공개',
      channel: '연합뉴스',
    },
    outlet_reports: [
  ...
  // 5. 제주 해녀 문화 유네스코 추가 등재 추진 — culture_sports (entity_card)
  '00000000-0000-4000-8000-000000000005': {
    ai_analysis: {
      ...
      entity_card: {
        ...
        domain_facts: [
          { label: '유네스코 등재', value: '2016년 인류무형문화유산', source: 'UNESCO ICH' },
          { label: '활동 인구', value: '약 4,000명', source: '제주도청 2024' },
          { label: '평균 연령', value: '70세 이상' },
          { label: '주요 활동지', value: '제주도, 우도, 마라도' },
        ],
      },
    },
    outlet_reports: [
  ...
  // 6. 삼성전자 1분기 반도체 흑자 전환 — economy (entity_card)
  '00000000-0000-4000-8000-000000000006': {
    ai_analysis: {
      ...
      entity_card: {
        ...
        domain_facts: [
          { label: '시가총액', value: '약 ₩460조', source: 'KRX 2026-04-30' },
          { label: 'CEO (DS부문)', value: '전영현 부회장' },
          { label: 'D램 점유율', value: '약 42%', source: 'TrendForce 2025-Q4' },
          { label: 'HBM3E 양산', value: '2024-Q3 본격 시작' },
          { label: '직원 수', value: '약 270,000명', source: '2024 사업보고서' },
        ],
      },
    },
    youtube_news: {
      video_id: 'YTN-news-06',
      title: '[YTN24] 삼성전자 반도체 1분기 흑자 전환…HBM이 끌었다',
      channel: 'YTN',
    },
    outlet_reports: [
  ...
  '00000000-0000-4000-8000-000000000007': {
    ...
    youtube_news: {
      video_id: 'MBC-news-07',
      title: '[MBC뉴스] 의·정 6개월 분쟁 끝…합의안 도출',
      channel: 'MBC뉴스',
    },
    outlet_reports: [
  ...
  // 8. 카카오 신규 AI 비서 베타 공개 — tech_science (entity_card)
  '00000000-0000-4000-8000-000000000008': {
    ai_analysis: {
      ...
      entity_card: {
        ...
        domain_facts: [
          { label: '시가총액', value: '약 ₩18조', source: 'KRX 2026-04-30' },
          { label: 'CEO', value: '정신아 대표' },
          { label: '카카오톡 MAU', value: '약 4,830만', source: '카카오 2024-Q4 IR' },
          { label: '핵심 자회사', value: '카카오뱅크, 카카오페이, 카카오모빌리티' },
        ],
      },
    },
    youtube_news: {
      video_id: 'YTN-news-08',
      title: '[YTN24] 카카오 AI 비서 베타 5만 명 돌파',
      channel: 'YTN',
    },
    outlet_reports: [
  ...
  '00000000-0000-4000-8000-000000000009': {
    ...
    youtube_news: {
      video_id: 'YHA-news-09',
      title: '[연합뉴스] 북, 동해상으로 미사일 2발 발사',
      channel: '연합뉴스',
    },
    outlet_reports: [
  ...
  // 10. 현대차 신형 전기차 사전예약 첫날 흥행 — economy (entity_card)
  '00000000-0000-4000-8000-000000000010': {
    ai_analysis: {
      ...
      entity_card: {
        ...
        domain_facts: [
          { label: '글로벌 순위', value: '3위 (현대차그룹)', source: 'OICA 2024' },
          { label: '시가총액', value: '약 ₩50조', source: 'KRX 2026-04-30' },
          { label: '대표', value: '장재훈 사장' },
          { label: '2024 EV 글로벌 판매', value: '약 53만 대', source: '현대차 IR' },
          { label: 'E-GMP 플랫폼', value: '아이오닉 5/6/9, EV6, GV60 공유' },
        ],
      },
    },
    youtube_news: {
      video_id: 'YTN-news-10',
      title: '[YTN24] 현대 신형 EV 첫날 4만 대 돌파',
      channel: 'YTN',
    },
    outlet_reports: [
  ...
  // 12. 쿠팡 새벽배송 권역 9개 도시 확대 — economy (entity_card)
  '00000000-0000-4000-8000-000000000012': {
    ai_analysis: {
      ...
      entity_card: {
        ...
        domain_facts: [
          { label: '시가총액', value: '약 $42B', source: 'NYSE 2026-04-30' },
          { label: '창업자·CEO', value: '김범석 (Bom Kim)' },
          { label: '활성 고객', value: '약 2,200만명', source: 'Coupang 2024-Q4 IR' },
          { label: '풀필먼트 센터', value: '국내 100여 개', source: '회사 공시' },
        ],
      },
    },
    outlet_reports: [
  ...
  // 13. 한국 영화 칸 영화제 진출 — culture_sports (entity_card)
  '00000000-0000-4000-8000-000000000013': {
    ai_analysis: {
      ...
      entity_card: {
        ...
        domain_facts: [
          { label: '창립', value: '1946년' },
          { label: '개최지', value: '프랑스 칸 (Côte d’Azur)' },
          { label: '최고상', value: '황금종려상 (Palme d’Or)' },
          { label: '한국 황금종려상', value: '봉준호 〈기생충〉 (2019)' },
          { label: '개최 시기', value: '매년 5월' },
        ],
      },
    },
    youtube_news: {
      video_id: 'YHA-news-13',
      title: '[연합뉴스] 한국 영화 2편, 칸 경쟁 부문 진출',
      channel: '연합뉴스',
    },
    outlet_reports: [
  ...
  // 14. KBO 흥행 5년 만에 최다 관중 경신 — culture_sports (entity_card)
  '00000000-0000-4000-8000-000000000014': {
    ai_analysis: {
      ...
      entity_card: {
        ...
        domain_facts: [
          { label: '창립', value: '1982년' },
          { label: '구단 수', value: '10개', source: 'KBO 공식' },
          { label: '시즌 일정', value: '3월 말 ~ 10월 (정규), 10~11월 (포스트시즌)' },
          { label: '한국시리즈 최다 우승', value: 'KIA 타이거즈 12회 (2024 기준)' },
          { label: '평균 관중', value: '경기당 약 12,000명 (2024 기준)' },
        ],
      },
    },
    youtube_news: {
      video_id: 'MBC-news-14',
      title: '[MBC뉴스] KBO 누적 관중 5년 만에 신기록',
      channel: 'MBC뉴스',
    },
    outlet_reports: [
  ...
  // 15. 동해 심해 가스전 시추 결과 — economy (entity_card) — insufficient_sample
  '00000000-0000-4000-8000-000000000015': {
    ai_analysis: {
      ...
      entity_card: {
        ...
        domain_facts: [
          { label: '설립', value: '1979년' },
          { label: '본사', value: '울산광역시' },
          { label: '주무 부처', value: '산업통상자원부' },
          { label: '핵심 사업', value: '석유 비축, 자원개발, 가스 운영' },
        ],
      },
    },
    outlet_reports: [
